...
#define LED_ON()    do { palSetPad(GPIOA, GPIOA_LED_GREEN) ;} while (0)
#define LED_OFF()   do { palClearPad(GPIOA, GPIOA_LED_GREEN); } while (0)
#define LED_TGL()   do { palTogglePad(GPIOA, GPIOA_LED_GREEN); } while (0)
...
static matrix_row_t read_cols(void)
{
    return ((palReadPad(GPIOC, GPIOC_BUTTON)==PAL_LOW) ? 0 : (1<<0));
